...
public class InteractableWater : MonoBehaviour
{
    ...
    // Fits the parent's BuoyancyEffector2D volume (BoxCollider2D + surface level) to this water mesh,
    // so the physics surface matches the visual surface. Works with any scale on this object.
    public void FitEffectorCollider()
    {
        Transform parent = transform.parent;
        BoxCollider2D box = parent != null ? parent.GetComponent<BoxCollider2D>() : null;
        BuoyancyEffector2D effector = parent != null ? parent.GetComponent<BuoyancyEffector2D>() : null;
        if (box == null || effector == null)
        {
            UnityEngine.Debug.LogWarning("[InteractableWater] Parent needs a BoxCollider2D and a BuoyancyEffector2D.", this);
            return;
        }

        // Mesh corners -> world -> parent local (the space the box lives in)
        Vector2 min = parent.InverseTransformPoint(transform.TransformPoint(new Vector3(-Width / 2, -Height / 2, 0f)));
        Vector2 max = parent.InverseTransformPoint(transform.TransformPoint(new Vector3(Width / 2, Height / 2, 0f)));

        box.offset = (min + max) / 2f;
        box.size = new Vector2(Mathf.Abs(max.x - min.x), Mathf.Abs(max.y - min.y));
        box.isTrigger = true;
        box.usedByEffector = true;

        // Surface level is an offset from the effector's transform: the top of the box
        effector.surfaceLevel = box.offset.y + box.size.y / 2f;
    }
    ...
    public void GenerateMesh()
    {
        ...
        // sharedMaterial: .material in edit mode leaks a new material copy on every regenerate
...
// Editor-only: UnityEditor does not exist in a player build
#if UNITY_EDITOR
...
public class InteractableWaterEditor : Editor
{
    ...
    public override VisualElement CreateInspectorGUI()
    {
        ...
        Button fitEffectorButton = new Button(() =>
        {
            // Record for Undo - this also marks the scene dirty so the change gets saved
            Transform parent = _water.transform.parent;
            if (parent != null && parent.TryGetComponent(out BoxCollider2D box) && parent.TryGetComponent(out BuoyancyEffector2D effector))
            {
                Undo.RecordObjects(new Object[] { box, effector }, "Fit Effector Collider");
            }
            _water.FitEffectorCollider();
        })
        {
            text = "Fit Effector Collider"
        };
        root.Add(fitEffectorButton);
