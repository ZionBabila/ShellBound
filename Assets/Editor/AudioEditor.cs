...
public class AudioEditor : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        // When a new clip is picked, make sure its raw samples are readable. GetData()
        // throws on Streaming/compressed clips, so force DecompressOnLoad import settings.
        if (EditorGUI.EndChangeCheck() && audioClip != null)
        {
            EnsureClipReadable(audioClip);
            waveformTexture = null; // Rebuild the waveform for the newly selected clip.
        }
    ...
    // Ensures a clip's samples can be read with GetData(). Streaming (and some compressed)
    // clips throw on GetData, so this switches the asset to DecompressOnLoad + preload and
    // reimports it. Short SFX should use DecompressOnLoad anyway; Streaming is for long music.
    private void EnsureClipReadable(AudioClip clip)
    {
        string path = AssetDatabase.GetAssetPath(clip);
        if (string.IsNullOrEmpty(path)) return; // Runtime-created clip, nothing to reimport.
        ...
        if (changed)
        {
            ...
            Debug.Log($"AudioEditor: '{clip.name}' was not readable (Streaming/compressed). Switched to DecompressOnLoad so it can be trimmed/faded.");
        }
    }
